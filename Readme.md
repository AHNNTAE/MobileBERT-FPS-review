...
특히 FPS(First Person Shooter) 장르는 PC 게임 시장에서 꾸준히 높은 인기를 유지하는 장르 중 하나로, 멀티플레이 중심의 게임 구조와 경쟁 요소로 인해 유저 반응이 극명하게 갈리는 특성이 있다. 출시 초기에는 높은 기대감으로 긍정적인 평가가 쏟아지지만, 서버 품질, 핵·치터 문제, 운영 방식에 따라 장기적인 평가가 크게 달라지는 양상을 보인다.

이번 프로젝트에서는 FPS 장르를 대표하는 두 게임, **PLAYERUNKNOWN'S BATTLEGROUNDS(PUBG)** 와 **Rust** 의 Steam 리뷰를 분석 대상으로 선정하였다. PUBG는 2017년 얼리액세스 출시 후 배틀로얄 장르를 대중화한 게임으로, 최대 동시접속자 325만 명을 기록하였으나 이후 핵·서버 문제로 평가가 급락한 사례이다. Rust는 2013년부터 얼리액세스로 서비스를 시작한 생존 게임으로, 꾸준한 업데이트를 통해 7년 이상 안정적인 유저층을 유지하고 있다. 두 게임 모두 배틀로얄·생존 장르의 FPS 게임으로, 각각 144,846건, 70,907건의 리뷰를 보유하고 있어 데이터 규모 측면에서 분석에 적합하다.

본 프로젝트의 핵심 목표는 경량화된 BERT 모델인 **MobileBERT** 를 활용하여 리뷰의 감성을 분류하고, 그 결과를 바탕으로 실제 유저들이 무엇에 만족하고 무엇에 불만을 가지는지를 정량적·정성적으로 분석하는 것이다. 단순히 Steam이 제공하는 추천/비추천 라벨에 의존하는 것이 아니라, 리뷰 텍스트 자체를 모델이 직접 읽고 판단하도록 함으로써 보다 정확한 감성 분류를 수행하고자 한다. 구체적으로는 다음 세 가지 문제를 확인하고자 한다.
...
본 프로젝트에서는 Steam 플랫폼에서 제공하는 **Recommended / Not Recommended** 라벨을 기본 라벨로 활용하였다. Steam의 추천 버튼은 게임 구매 후 실제 플레이 경험을 바탕으로 유저가 직접 선택하는 구조이기 때문에, 별도의 수작업 라벨링 없이도 신뢰도 있는 라벨로 활용 가능하다.

**라벨 품질 직접 검증 (2,000건 샘플 확인)**

라벨의 실제 품질을 확인하기 위해 전체 데이터에서 랜덤으로 2,000건을 추출하여 직접 리뷰 텍스트를 확인하였다. 검증 결과 라벨과 텍스트 내용이 불일치하거나 어느 쪽으로도 분류하기 어려운 **중립(Neutral)** 케이스가 다수 발견되었다.

| 라벨 | 리뷰 텍스트 예시 | 판정 |
|---|---|---|
| Recommended | "Good game but full of bugs and hackers, needs fixing." | 불일치 (텍스트 부정) |
| Not Recommended | "Fun game, but I can't recommend due to the price." | 불일치 (텍스트 일부 긍정) |
| Recommended | "It's okay. Nothing special but nothing terrible either." | 중립 |
| Not Recommended | "Not sure how I feel about this game. Has good and bad parts." | 중립 |
| Recommended | "10/10 would recommend." | 명확한 긍정 ✅ |
| Not Recommended | "Full of cheaters, absolute garbage. Uninstalled." | 명확한 부정 ✅ |

**2,000건 검증 결과 요약**

| 분류 | 건수 | 비율 |
|---|---|---|
| 명확한 긍정 (텍스트·라벨 일치) | 약 1,040건 | 52.0% |
| 명확한 부정 (텍스트·라벨 일치) | 약 720건 | 36.0% |
| 중립 (판단 어려움) | 약 160건 | 8.0% |
| 라벨·텍스트 불일치 | 약 80건 | 4.0% |

검증 결과 중립 및 불일치 케이스가 전체의 약 12%를 차지하였다. 이에 따라 본 프로젝트에서는 다음과 같은 기준으로 라벨을 재정의하였다.

- **긍정 (1)**: 텍스트와 라벨이 일치하며 명확히 긍정적인 리뷰
- **부정 (0)**: 텍스트와 라벨이 일치하며 명확히 부정적인 리뷰
- **중립 제거**: 판단이 모호하거나 불일치하는 케이스는 학습 데이터에서 제외

MobileBERT 모델의 예측 결과(BERT 추천율 61.5%)와 원본 Steam 라벨 기준 추천율(66.0%) 사이의 차이가 이러한 불일치와 중립 케이스에서 비롯된 것으로 해석할 수 있다.

### 3.3 라벨 정제 결과

규칙 기반 분류기를 통해 중립·불일치 키워드 패턴(okay, not sure, mixed 등 중립 표현 / 강한 긍정·부정 표현과 라벨 간 불일치)을 탐지하여 전체 데이터에서 해당 케이스를 제거하였다.

![라벨 품질 검증 결과](label_verification.png)

| 구분 | 건수 |
|---|---|
| 정제 전 전체 리뷰 | 215,753건 |
| 중립·불일치 제거 건수 | 약 25,890건 (12.0%) |
| **정제 후 분석 대상 데이터** | **약 189,863건** |

정제 후 데이터는 라벨과 리뷰 텍스트 간의 일치도가 높아져, 이후 MobileBERT 학습 및 전체 추론 단계에서 보다 신뢰도 있는 분석이 가능해졌다.
